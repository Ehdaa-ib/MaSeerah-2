...
/** Keep in sync with lib/core/validators.dart and firestore.rules isAdminEmail(). */
...
exports.sendFeedbackReply = onCall({ region: callableRegion }, async (request) => {
  ...
  const senderEmail = normalizeEmail(request.auth.token.email || adminEmail);
  ...
  try {
    ...
    if (e instanceof HttpsError) throw e;
    throw new HttpsError('internal', 'Could not send email. Check SMTP configuration.');
  }
